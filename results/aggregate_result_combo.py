...
avg_df_original = combined_df_original.groupby(['cell_type','condition','dose', 'n_top_deg']).mean().reset_index() #, 'cell_type'
...
for reg_rate in intense_reg_rates:
    for p in intense_p_values:
        ...
        avg_df = combined_df.groupby(['cell_type','condition','dose', 'n_top_deg']).mean().reset_index() #, 'cell_type'
